...
#parser.add_argument("--N_READS", help="Number of reads in the sample")
...
#
 ...
 def output(self, outputfile):
  with open(PyPluMA.prefix()+"/"+self.parameters["SAM"], 'r') as f:
    coverage_dict = {}
    length_dict = {}
    hits_per_million = {}
    for row in f.readlines():
        if ("@HD" in row) or ("@PG" in row):
            ...
            hits_per_million[genome] += 1000000 / int(self.parameters["N_READS"]) # (1/N_READS is number of hits for one read
    ...
    coverage_df = coverage_df[coverage_df["coverage"]>float(self.parameters["COV_CUTOFF"])]
    ...
    coverage_df[["genome", "coverage", "abundance", "hits_per_million"]].to_csv(outputfile, sep="\t", index=False)
